...
def find_noise(G, vgg16, target_image, device, num_steps=100, init_lr=0.1,
    ...
    w_std = np.std(w_samples, axis=0, keedims=True)
    ...
    target_image = torch.tensor(target_image, dtype=torch.float32, device=device).unsqueeze(0)
    ...
    for step in tqdm(range(num_steps)):
        optimizer.zero_grad()
        w_noise_scale = w_std * std_factor * max(0.0, 1.0 - 2 * step / num_steps) ** 2
        w_noise = torch.randn_like(w_opt) * w_noise_scale
        ws = (w_opt + w_noise).repeat([1, G.mapping.num_ws, 1])
        synth_image = G.synthesis(ws, noise_mode='const')

        synth_features = get_features(synth_image)
        perceptual_loss = l2_criterion(target_features, synth_features)
